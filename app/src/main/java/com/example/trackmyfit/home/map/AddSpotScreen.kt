...
fun AddSpotScreen(navController: NavHostController, latitude: Double?, longitude: Double?, viewModel: SpotViewModel = viewModel()) {
    ...
    val activityOptions = listOf("Running", "Cycling", "Rollerblade", "Hiking", "Gym", "Outdoor gym")
                    ...
                    Button(
                        onClick = {
                            if (spotName.isNotBlank() && latitude != null && longitude != null) {
                                ...
                                viewModel.saveWorkoutSpot(newSpot) { generatedId ->
                                    // Once the spot is saved, use the generatedId
                                    navController.popBackStack() // Navigate back to the map
